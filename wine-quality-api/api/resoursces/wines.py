...
class getMaxFixedAcidicity(Resource):
    ...
    def get(self):
        winelist = WineModel.query.order_by(WineModel.fixed_acidity).filter_by(WineModel.fixed_acidity == (WineModel.query(WineModel.fixed_acidity).order_by(WineModel.fixed_Acidity).desc().limit(1)))
        datalist = []
...
class getNeutralWines(Resource):
    ...
    def get(self):
        winelist = WineModel.query.filter_by(WineModel.pH > 6, WineModel.pH < 8)
        datalist = []
        for item in winelist:  
            temp = item.json()
            datalist.append(temp)
...
class getTopAlchohol(Resource):
    ...
    def get(self):
        winelist = WineModel.query.order_by(WineModel.alchohol.desc()).limit(10)
        datalist = []
        for item in winelist:  
            temp = item.json()
            datalist.append(temp)
...
class getBottomDensityWines(Resource):
    ...
    def get(self):
        winelist = WineModel.query.order_by(WineModel.density).limit(10)
        datalist = []
        for item in winelist:  
            temp = item.json()
            datalist.append(temp)
